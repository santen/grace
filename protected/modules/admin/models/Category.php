...
class Category extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, division, tablename, cdate', 'required'),
			array('parent_id, division, is_remove', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>80),
			array('tablename', 'length', 'max'=>60),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, parent_id, division, tablename, cdate, is_remove', 'safe', 'on'=>'search'),
		);
	}
